...
- [Done] Introduced shared desk experience for `Opportunity Desk` and `Legal Desk` with unified utility headers and minimal modern card/hero styling (`portal-hub.html`, `legal-hub.html`, `css/portal-expansion.css`, `js/utility-header.js`).
- [Done] Refined legal document pages to a continuous minimal "paper sheet" layout (single centered document surface, reduced visual noise, shared typography rhythm) via `css/main.css` without changing legal content markup.
- [Done] Converted `Legal Desk` hub document links from card pods to a single-column continuous list layout with simple row separators (no boxed tiles) via `legal-hub.html` and `css/portal-expansion.css`.
- [Done] Removed remaining Legal Desk/legal-page card surfaces and atmospheric overlays; legal pages now use a flat, continuous light background with unboxed single-column content and no pod containers.
- [Done] Restored continuous paper-style content surface for legal documents and Legal Desk while keeping pods/cards removed (single-sheet layout over neutral page background).
- [Done] Added subtle all-edge paper border/shadow and tucked legal paper under shared utility header; added light global shadow treatment to `.tsi-utility-bar` for consistent depth on pages using this header.
- [Done] Retuned legal paper elevation styling: removed edge border/outline and switched to a darker, tighter drop-shadow profile (shorter distance, higher opacity) for cleaner sheet edges.
- [Done] Applied the same neutral background + tucked paper-surface treatment to `Opportunity Desk` (`portal-hub`) with matching compact darker shadow and no edge border.
- [Done] Fixed utility-header hover menu gap by aligning logo and breadcrumb dropdown menus flush to their trigger (`top: 100%`) so options remain open when moving pointer from trigger to menu.
- [Done] Unified `Legal Desk` and `Opportunity Desk` styling: same neutral background, same tucked paper surface/shadow, unboxed hero treatment, and single-column no-card content rows with inline link actions.
- [Done] Set Opportunity hub and route pages (`portal-hub`, `portal-investment`, `portal-press`, `portal-employment`, `portal-internship`) to default light theme via HTML root attributes while preserving runtime theme toggle.
- [Done] Removed lingering Opportunity Desk title-card style by replacing later hero override with fully unboxed hero styling (plain heading block, no border/background/shadow).
- [Done] Renamed Opportunity path to Careers (`portal-hub` and related labels), removed Press from Careers hub/submenu routes, and promoted Press to standalone header/menu item while preserving shared utility-header style/behavior.
- [Done] Replaced file-upload UI across portal intake routes with a unified drag-and-drop upload component (dashed drop zone, select-file affordance, filename state, drag-over feedback) via shared runtime enhancement in `js/portal-intake-v2.js` and shared styles in `css/portal-expansion.css`.
- [Done] Applied the same paper/no-card layout system to all Careers/Press individual route pages (`portal-investment`, `portal-press`, `portal-employment`, `portal-internship`): neutral backdrop, tucked paper shell, unboxed hero, and unboxed form container.
- [Done] Fixed Careers Desk breadcrumb hover submenu visibility by removing clipping on utility-title container (`.tsi-utility-bar__title` no longer uses `overflow: hidden`), allowing submenu options to render.
- [Done] Upload/header polish pass: fixed persistent upload `X` visibility (`[hidden]` override), centered upload plus-icon geometry, and unified dropdown menu typography/hover styling between logo and breadcrumb menus.
- [Done] Improved global utility-header submenu usability by adding hover persistence (`.is-open` + short close delay) so users can move diagonally from trigger to dropdown options without menu collapse.
- [Done] Portal form UX/logic refinement pass: added inline example placeholders, auto required-field notation markers, internship graduation-label clarification, employment "Open to multiple paths" auto-role behavior, investment investor-type dropdown with conditional "Other" input, press "No deadline" mode, and stronger regex email validation in both portal and main form submit logic.
- [Done] Careers flow restructure: replaced `portal-employment.html` intake route with a no-form `Job Descriptions` page, added new `portal-career-interest.html` as the employment/career-interest form route, updated Careers hub/link labels to `Job Descriptions`, and added dual-submit behavior on career-interest form (`Submit` and `Submit and view our internship opportunities`) with post-submit redirect support in `js/portal-intake-v2.js`.
- [Done] Removed card/pod presentation from `portal-employment.html` and replaced it with a flat job-listings pattern (search/filter rows, plain results list, and inline route links) scoped via `.portalx-jobs-page` styles in `css/portal-expansion.css`.
- [Done] Refined `portal-employment.html` to a closer enterprise job-board format: blue top search strip, left filter rail, right metadata/sort + row-based results area (no result cards), with all style changes scoped to `.portalx-jobs-page`.
- [Done] Stabilized Job Listings styling scope by extracting all `.portalx-jobs-page` jobboard rules into `css/job-listings.css`, removing remaining jobboard selectors from `css/portal-expansion.css`, and normalizing shared sidebar panel-title box model to prevent Search/Refine header padding drift.
- [Done] Standardized shared naming/routing labels with a central source (`js/site-labels.js`) consumed by utility headers/menus and main footer links; updated route naming to `Career Opportunities`, `Investment Inquiry`, and `Public Relations/Media Request`, removed `Job Listings`/`Internship Application`/`Legal Desk` from main footer links, split Investment out of Career hub navigation, and renamed security surfaces to `Security Statement`.
- [Done] Applied follow-up UX polish and Globe Lab controls expansion: fixed Job Listings helper-text padding, widened utility dropdown menus to prevent aggressive wrapping, delayed Overview paragraph backdrop reveal until after globe zoom-out phase, and upgraded Globe Lab with diagnostics readouts, drag-reorderable layers, country-level color/visibility overrides, hold-to-override rotation behavior, saved-settings load/save controls (auto-load default off), section dividers, and selectable standard/high-detail atlas source.
- [Done] Expanded utility-header hover behavior to support additional nested hover access from logo menus (for example `Career Opportunities` -> `Job Listings` / `Internship Application`) and rebuilt the main footer into a strict two-row left/center/right layout with requested linked/unlinked egg separators and Globe Lab egg placement.
- [Done] Refined Globe Lab layer controls to a single unified layer list: drag-order for base layers, click-to-toggle visibility for both base and override entries, inline `x` removal for manual override layers, override-add flow integrated directly into the layer area, and footer full-width grid alignment corrected so right-side content tracks the same edge margin band as left-side content.
- [Done] Fixed primary index nav glass persistence by promoting only the main site nav to fixed overlay mode (`body.has-primary-nav`) so globe content remains visible behind it, and tightened footer grid span constraints to restore balanced left/right edge alignment without collapsing inward.
- [Done] Globe Lab layer panel refinement: moved base/override color pickers into the same row as each layer entry, removed separate layer-color section, and tinted active layer controls using each layer’s current color for immediate visual state feedback.
- [Done] Navigation/footer/pipeline reliability pass: logo click now forces a true first-load overview replay cycle (distinct from `00` quick replay), footer theme toggle moved to bottom-row center, and pipeline map now has MD-fetch fallback to PNG (`data-map-src`) so marker overlays/tabs remain functional when MD fetch is unavailable.
- [Done] Pipeline + Globe Lab interaction corrections: removed duplicate phase-tab click handling (fixes desktop no-activation + mobile dual overlay markers), aligned frame glow color resolution with tab color tokens, enabled drag reordering for added override layers (override list ordering), removed layer drag-pill UI while keeping drag behavior, slimmed layer-row controls, added lab background controls (BG color + stars color + twinkle), and constrained settings persistence to manual Globe Lab save/load only (no auto-load behavior).
- [Done] Globe Lab header/UX pass: moved diagnostics into the Globe Lab top bar center (page-specific exception), added click-to-collapse helper hint behavior, and fixed draw-mode cursor scope so crosshair does not persist over the settings panel when draw mode is inactive.
...
- [Done] 2026-03-01: Reverted footer legal link styling back to the original minimalist inline-link behavior and moved Globe-Lab-style header bars to each footer destination page (legal + opportunity desk routes) for consistent top-of-page navigation framing.
- [Done] 2026-03-01: Updated Globe Lab state rendering to exclude Texas from the states layer so Texas color/visibility is independent, and changed default states stroke to neutral theme-aware tones.
- [Done] 2026-03-01: Fixed Globe Lab latitude control persistence by removing automatic latitude spring-back while spin is active; drag/slider latitude adjustments now persist until explicitly changed.
- [Done] 2026-03-01: Standardized footer-linked page headers to shared utility-bar behavior with logo + breadcrumb left rail and Globe-Lab-style right controls (`Theme` + `Back to Site`) across legal and opportunity-desk pages.
- [Done] 2026-03-01: Updated `Back to Site` behavior to route via `index.html?home=00#overview` and added launch handling in `js/main.js` (`home=00`) to run the same quick overview replay/globe reset behavior as clicking tab `00`.
- [Done] 2026-03-01: Added shared `js/utility-header.js` for consistent header theme toggling and home-link wiring; bumped active page CSS cache keys to `v=20260301-uihotfix` to eliminate stale-style divergence between legal and portal pages.
- [Done] 2026-03-01: Resolved remaining header inconsistency by removing hardcoded `data-theme=\"light\"` from legal pages, moving portal layout padding from `.portalx-page` to `.portalx-shell` (full-width top bar), and loading the same font set on portal/globe-lab pages for consistent header typography.
- [Done] 2026-03-01: Added central `legal-hub.html` (Legal Desk) patterned after Opportunity Desk and linked it from main footer navigation; updated legal and portal header left rails to clickable breadcrumb location links.
- [Done] 2026-03-01: Header left-location clickability is now consistent across this area (`TSI` thumbnail + clickable trail segments), while right controls remain unified (`Theme` + `Back to Site`) and back/home routes preserve Overview `00` replay behavior.
- [Done] 2026-03-01: Extended utility-header interactions so breadcrumb parent links with child routes expose hover/focus submenus (currently `Opportunity Desk` => 4 route pages and `Legal Desk` => 4 legal pages) via shared JS/CSS (`js/utility-header.js`, `css/main.css`).
...
- [Done] 2026-02-27: Tuned overview wireframe/globe background responsiveness in `css/main.css` with viewport-aware sizing/offset tokens, mobile/tablet intensity scaling, and a readability scrim so hero copy stays legible across desktop and mobile views.
- [Done] 2026-02-27: Added targeted micro-breakpoint tuning for the overview wireframe (`<=420px` and short-landscape phone cases) and refined base/tablet/mobile animation intensity values to reduce visual dominance while preserving background motion presence.
- [Done] 2026-02-27: Applied visibility-first overview background update by increasing animation opacity/reducing scrim strength across breakpoints and removing the `prefers-reduced-motion` hard-hide rule (now reduced-opacity instead) so the wireframe remains visible in all modes.
- [Done] 2026-02-27: Enabled temporary overview full-brightness test mode (`css/main.css`) by setting animation opacity to `1` across breakpoints/reduced-motion and disabling the scrim overlay so background visibility can be validated quickly.
- [Done] 2026-02-27: Updated overview animation to fullscreen fill mode by switching the background iframe layer to fixed viewport sizing (`100vw` x `100svh`) so it fills the screen during overview testing.
- [Done] 2026-02-27: Converted `assets/pages/overview/globe-3.7.2-west.html` from fixed `400x400` rendering to viewport-responsive rendering (`100vw` x `100svh` with resize-driven projection/viewBox updates), eliminating square-window behavior on desktop and height underfill on mobile.
- [Done] 2026-02-28: Rebuilt `assets/pages/overview/globe-3.7.2-west.html` with staged intro choreography (single-color start, Texas-first reveal, then full world detail), nonlinear `slow -> fast -> slow` zoom easing, and dual-mode scaffold (`?mode=orbit` default + optional `?mode=cockpit`).
- [Done] 2026-02-28: Added overview-globe performance guardrails (mobile/reduced-motion dot budgets, device-aware frame throttling, visibility-aware render gating for non-`#overview` states, bounded point-sampling attempts) to reduce unnecessary runtime load.
- [Done] 2026-02-28: Updated overview iframe wiring in `index.html` to load `assets/pages/overview/globe-3.7.2-west.html?mode=orbit`, keeping full-frame behavior while locking baseline mode for predictable visual alignment.
- [Done] 2026-02-28: Disabled overview dots/particles by default while preserving particle logic behind query toggle (`particles=on`), so future particle systems can be re-enabled without refactoring.
- [Done] 2026-02-28: Added local atlas data bundle under `assets/pages/overview/data/` and wired source toggle support (`source=local` or `source=cdn`) with fallback loading; overview now defaults to local source in `index.html`.
- [Done] 2026-02-28: Added overview cockpit camera orientation controls (`pitch`, `roll`) in `globe-3.7.2-west.html`, with cockpit defaults aligned to a straight-down/north-oriented feel (`cockpitStartPitch=-90`, `cockpitStartRoll=-90`) and intro interpolation into runtime camera state.
- [Done] 2026-02-28: Added overview camera heading control (`yaw`) in `globe-3.7.2-west.html`, blended late in intro to preserve Texas-first reveal and carried into post-intro orbit/cockpit motion for full yaw/pitch/roll tuning.
- [Done] 2026-02-28: Added in-file camera helper documentation to `globe-3.7.2-west.html` (param reference, orientation baseline example, keyframe-sequence template) plus optional `help=on` console hint output for rapid tuning.
- [Done] 2026-02-28: Updated overview camera angle semantics so `pitch`/`roll` are relative by default from straight-down baseline (`angles=relative`), with explicit absolute override support via `angles=absolute`.
- [Done] 2026-02-28: Added standalone overview animation test rig `assets/pages/overview/globe-3.7.3-west.html` with embedded live controls for `yaw`, `pitch`, `roll`, `angles (relative|absolute)`, `zoom`, and `rotation speed`; no main-site wiring changes.
- [Done] 2026-02-28: Fixed yaw responsiveness in `globe-3.7.3-west.html` by applying yaw continuously after intro (as a persistent heading offset) with drift accumulation layered on top, so live yaw edits are visible immediately.
- [Done] 2026-02-28: Corrected `globe-3.7.3-west.html` relative-angle baseline to true straight-down-over-Texas orientation (`pitch=-31`, `roll=0` reference) instead of cockpit starter pose, resolving yaw-insensitive control behavior at pole-like states.
- [Done] 2026-02-28: Added `assets/pages/overview/globe-3.7.4-west.html` as a satellite-perspective test harness (`d3-geo-projection` `geoSatellite`) for off-center camera behavior, with live controls for yaw/pitch/roll, relative/absolute mode, distance (zoom), camera tilt, and rotation speed.
- [Done] 2026-02-28: Added `assets/pages/overview/globe-3.8.0-webgl.html` minimal Three.js prototype with true camera-space controls (yaw/pitch/roll, relative/absolute mode, camera distance, rotation speed), Texas-first intro hooks, and no main-site wiring changes.
- [Done] 2026-02-28: Removed center-aim lock in `globe-3.8.0-webgl.html` by replacing fixed `lookAt(0,0,0)` with live-configurable look target offsets (`lookX/lookY/lookZ`), enabling true off-center camera aiming.
- [Done] 2026-02-28: Added overview globe alias asset `assets/pages/overview/globe-v3.7e.html` (from existing `3.7e` runtime) and switched `index.html` overview iframe source to this requested filename.
- [Done] 2026-02-28: Applied background position/zoom action to `assets/pages/overview/globe-v3.7e.html`: intro now auto-runs `slow -> fast -> slow` zoom-out (`introStartZoom` to `introEndZoom`) and transitions into gentle post-intro earth rotation (`postIntroRollDegPerSec`).
- [Done] 2026-02-28: Resolved competing intro controls in `globe-v3.7e.html` by disabling wheel-zoom input path for background playback (`userWheelZoomEnabled=false` default) and isolating intro runtime state (`introActive`) so intro curve and post-intro roll do not fight legacy handlers.
- [Done] 2026-02-28: Hardened `globe-v3.7e.html` against apparent pan drift by switching to viewport-sized SVG (`100vw`/`100svh` + dynamic width/height/viewBox), setting Texas anchor to `[-99.5, 31]`, disabling pointer input on SVG, and cache-busting overview iframe source in `index.html`.
- [Done] 2026-02-28: Stripped `globe-v3.7e.html` to minimal `v3.7.min` runtime: retained only Texas-centered orthographic projection, world/country/Texas paths, responsive viewport sizing, intro `slow -> fast -> slow` zoom-out, and gentle post-intro roll; removed legacy twinkle/night/FX/helper/interactive zoom systems.
- [Done] 2026-02-28: Added staged data-load profiles to `globe-v3.7e.html` (`profile=texas-only|regional|global`) so the overview background can fetch/render only required datasets for expected framing; optional `promoteToGlobalAfterIntro` hook added for deferred countries load.
- [Done] 2026-02-28: Set active overview background loading to globe+Texas only by default (`loadProfile: texas-only` in `globe-v3.7e.html`) and updated `index.html` iframe query to `profile=texas-only` with cache-bust refresh.
- [Done] 2026-02-28: Applied fast-intro performance tuning in `globe-v3.7e.html` by shortening intro duration (`4200ms`) and suppressing graticule rendering during intro (restored post-intro), reducing early-frame detail workload.
- [Done] 2026-02-28: Set post-intro roll to zero in `globe-v3.7e.html` (`postIntroRollDegPerSec: 0`) and enabled deferred country-line loading after intro (`promoteToGlobalAfterIntro: true`) while keeping initial `texas-only` startup profile.
- [Done] 2026-02-28: Strengthened deferred countries visibility in `globe-v3.7e.html` by adding robust local->cdn fallback load path after intro, explicit country-stroke emphasis, and timed opacity fade-in so post-intro country-load is visibly distinct.
- [Done] 2026-02-28: Fixed partial country-border reveal in `globe-v3.7e.html` by switching deferred countries render from boundary mesh to full country outlines (`topojson.feature`) and loading land + countries together after intro for complete coast/border context.
- [Done] 2026-02-28: Increased post-intro geography visibility in `globe-v3.7e.html` by widening final zoom (`introEndZoom: 1.55`), enabling subtle land fill, and strengthening country stroke contrast/width (including deferred-load emphasis) so countries are clearly visible after intro.
...
- [Done] 2026-02-27: Reordered mobile pipeline layout so map frame renders between title strip and unified tab panel (`title -> map -> tabs`) by inserting the tab panel after the frame in `js/main.js`.
- [Done] 2026-02-27: Enforced mobile visual order for unified tab panel by setting `.pipeline-map-tab-panel { order: 3; }` so tab panel renders below map frame even when container order rules override DOM position.
- [Done] 2026-02-27: Updated mobile locations overlay interaction so `View Locations` label remains static (no `Hide Locations` swap) and raised overlay stacking above the button (`.pipeline-map-tab-locations-overlay` > `.pipeline-map-tab-locations-toggle`) so popup content can cover the button while open.
- [Done] 2026-02-27: Added pre-reveal input lock for pipeline control surfaces: desktop phase buttons and mobile tab panel now dim and ignore input until map startup animation signals completion (`mapInitialRevealComplete`), then automatically unlock.
- [Done] 2026-02-27: Delayed desktop preload unlock until Texas deferred override pass completes by introducing desktop-only lock state (`is-desktop-preload-locked`) released on new `mapTexasOverrideComplete` event (`mapTexasOverrideReady`), so desktop controls stay dimmed/disabled until Texas is fully loaded.
- [Done] 2026-02-27: Hardened desktop unlock timing to Texas visual completion by polling for no remaining deferred/entry Texas dot classes (`.map-dot--deferred-load[data-map-deferred-override=\"texas\"]` and `.map-dot--texas-entry`) before releasing lock; extended desktop safety fallback to 7000ms.
- [Done] 2026-02-27: Anchored desktop unlock timing to Texas override start by emitting `mapTexasOverrideStarted` + `mapTexasOverrideStartTs` when deferred Texas entry begins and requiring a minimum elapsed dwell (`1000ms`) after start before desktop lock can release.
- [Done] 2026-02-27: Enabled desktop rendering of the unified mobile tab-panel UI without removing existing desktop controls, and moved tab-panel insertion to sit between desktop buttons and map frame (`title -> desktop buttons -> tab panel -> map`) while leaving mobile-specific controls/layout logic intact.
- [Done] 2026-02-27: Corrected desktop tab-panel behavior parity by routing tab map-state checks/toggles through desktop overlay targets (`--desktop`), syncing desktop control clicks back to tab `is-map-active` state, and applying desktop right-pane/tab color state classes (`is-selected-map-on/off`, `has-content-active`, `is-map-pane-active`) to match mobile logic without changing mobile rules.
- [Done] 2026-02-27: Desktop-only locations behavior update for tab panel: left-justified `View Locations` button, disabled desktop popup overlay, and inline locations list reveal below existing pane text on click; mobile popup behavior unchanged.
- [Done] 2026-02-27: Fixed pipeline map frame inset consistency by setting `.pipeline-map-frame { box-sizing: border-box; }` so frame aspect-ratio sizing includes padding/border and avoids bottom-edge visual mismatch.
- [Done] 2026-02-27: Reduced residual map bottom-row clipping by changing `.pipeline-map-frame svg` from `height: auto` to `height: 100%` (with `max-height: 100%`) to avoid fractional auto-height rounding inside the fixed-ratio frame.
- [Done] 2026-02-27: Removed prior `.pipeline-map-frame { box-sizing: border-box; }` adjustment after it introduced left/right blank side gutters with full-height SVG fit; retained SVG `height: 100%` fix for bottom-row clipping.
- [Done] 2026-02-27: Added map render viewport bleed (`viewInset = 0.5`) in `js/main.js` by expanding SVG `viewBox` and ocean rect bounds, preventing right/bottom edge dot clipping from viewport rounding without introducing side gutters.
- [Done] 2026-02-27: Added a temporary desktop-only passive comparison map below the primary pipeline map (`data-map-compare`) using Monday geometry mode (`data-map-geometry-mode=\"monday-0223\"`) with permanent glow styling, while leaving the primary map and mobile behavior unchanged.
- [Done] 2026-02-27: Isolated passive comparison-map runtime in `js/main.js` so it skips control/tab/helper lock lifecycle and toggle UI generation (`data-map-passive=\"true\"`), preventing interference with primary map interaction state.
- [Done] 2026-02-27: Added a third desktop-only passive comparison map sourced from February 19 geometry mode (`data-map-geometry-mode=\"feb19-e8a21b9\"`) using the same compare-map logic and permanent glow treatment for side-by-side-in-sequence validation.
- [Done] 2026-02-27: Removed temporary pipeline map comparison harness (2nd/3rd maps in `index.html` plus compare-only CSS/JS branches) to eliminate diagnostic side effects and return to single-map runtime path.
- [Done] 2026-02-27: Restored primary map render geometry to Feb 19 behavior by setting `renderDots` viewBox/ocean bounds to exact `0 0 width height` (no inset branch) and returning `.pipeline-map-frame svg` sizing to `height: auto`.
- [Done] 2026-02-27: Added a minimal dot-grid vertical nudge in `js/main.js` (`dotCenterYOffset = 0.47`) so bottom row dots clear device-pixel rounding at frame edge without reintroducing side-gutter layout changes.
- [Done] 2026-02-27: Removed `.pipeline-map-frame` inner padding (base and small-screen override) to eliminate content-box ratio distortion that was clipping the map bottom row.
- [Done] 2026-02-27: Added typewriter-style character-by-character reveal for tab content body copy (`.pipeline-map-tab-pane-text` and `.pipeline-map-tab-pane-lead`) on pane activation, without changing pane title/underline styling or `View Locations` behavior.
- [Done] 2026-02-27: Removed tab-pane typewriter animation and restored normal text rendering; moved map helper copy into map tab pane body (`.pipeline-map-tab-pane-helper`) so it shows on MAP tab and hides when any other tab is selected, without changing title bar or `View Locations` behavior.
- [Done] 2026-02-27: Updated unified phase tab label text from `PHASE` to `PHASE TAB` in shared tab label builder so the change applies consistently to both desktop and mobile tab panels.
- [Done] 2026-02-27: Removed legacy desktop pipeline button lane creation (`.pipeline-map-inline-controls-desktop` and `.map-control--desktop`) from `js/main.js` so desktop view uses the tab panel as the active control surface while preserving mobile behavior.
- [Done] 2026-02-27: Moved desktop pipeline title rendering into the MAP tab content pane (reusing `pipeline-map-title-box` styling), removed standalone title-box injection above the map, and hid the legacy below-map `.pipeline-note` box on desktop.
- [Done] 2026-02-27: Removed legacy pipeline note box markup from `index.html` to eliminate mobile flash, added fallback MAP-pane overview copy in `js/main.js`, removed auto-guided first-tab highlight behavior, and changed tab `:focus-visible` outline to accent-only (no white mix) for both desktop and mobile.
- [Done] 2026-02-27: Removed remaining map-load reveal hook that forcibly showed `.pipeline-map-helper-overlay` text after startup, so helper copy now stays removed from the map surface while tab-pane helper behavior remains available.
- [Done] 2026-02-28: Rebuilt `assets/pages/overview/globe-v3.7e.html` atlas-loading flow into strict staged sequence: prefetch Texas (`states`) for intro, complete zoom-out, then fetch/fade land, then fetch/fade countries (each stage with local/CDN fallback) and updated overview iframe source token in `index.html` to `v=20260228g`.
- [Done] 2026-02-28: Converted `assets/pages/overview/globe-v3.7e.html` staged loader into a keyframe/timeline sequence model (`TIMELINE` + markers) while preserving current behavior order: intro zoom/Texas reveal -> fetch land/fade -> fetch countries/fade.
- [Done] 2026-02-28: Added centralized sequence tuning block (`SEQUENCE`) in `assets/pages/overview/globe-v3.7e.html`; timeline entries now reference this single config so fade timings/order/easing can be adjusted without changing processing logic.
- [Done] 2026-02-28: Simplified timeline tuning surface in `assets/pages/overview/globe-v3.7e.html` to a flat single-line definitions block (`SEQ`) so all sequence IDs/timings/markers/easing can be adjusted from one basic area without nested config traversal.
- [Done] 2026-02-28: Added globe center controls (`centerLon`, `centerLat`) to the same flat `SEQ` tuning block in `assets/pages/overview/globe-v3.7e.html` and wired projection anchor to these values.
- [Done] 2026-02-28: Removed duplicate sequence/camera timing fields from `OPT` in `assets/pages/overview/globe-v3.7e.html`; `SEQ` is now the single source of truth for center/zoom/roll/fade timing controls.
- [Done] 2026-02-28: Unified overview controls into one flat `CFG` block in `assets/pages/overview/globe-v3.7e.html` (camera, style, sequence, and lon/lat graticule generation timing/style), including timeline-driven graticule fade controls.
- [Done] 2026-02-28: Added per-section enable switches in `assets/pages/overview/globe-v3.7e.html` (`enableSphere/Texas/Graticule/Land/Countries`) so timeline steps and fetches can be fully disabled per element.
- [Done] 2026-02-28: Added per-element render-mode controls in `assets/pages/overview/globe-v3.7e.html` (`border|fill|both|none`) for sphere, Texas, graticule, land, and countries, with runtime style application and render gating.
- [Done] 2026-02-28: Added countries reveal toggle in `assets/pages/overview/globe-v3.7e.html` (`countriesRevealMode: single|groups`) with configurable random 5-group staged fade (`countriesGroupCount`, `countriesGroupStaggerMs`) while preserving single-block reveal as the default behavior.
- [Done] 2026-02-28: Added per-stage camera control sets in `assets/pages/overview/globe-v3.7e.html` (`intro/texas/graticule/land/countries Cam*`) for optional lon/lat/roll/zoom offset tweens, all disabled by default.
- [Done] 2026-02-28: Added per-stage camera persistence controls in `assets/pages/overview/globe-v3.7e.html` (`*CamPersist`, `*CamResetDurationMs`) so each stage can either hold its final offset or auto-reset to neutral.
- [Done] 2026-02-28: Added inline helper documentation in the `CFG` stage-camera section of `assets/pages/overview/globe-v3.7e.html` (offset units, stacking behavior, persistence/reset logic, and stage-timing mapping).
- [Done] 2026-02-28: Added infinite persistent-motion controls in `assets/pages/overview/globe-v3.7e.html` (`persistentMotionEnabled`, start marker, lon/lat/roll/zoom rates per second) to support continuous post-stage movement until overridden.
- [Done] 2026-02-28: Expanded inline helper documentation across `CFG` sections in `assets/pages/overview/globe-v3.7e.html` (switch behavior, render-mode impact, marker usage, fetch/fade staging, and persistent-motion guidance).
- [Done] 2026-02-28: Added region-priority countries grouping controls in `assets/pages/overview/globe-v3.7e.html` (`countriesGroupStrategy`, `countriesRegionOrderCsv`, `countriesRegionDelayMs`) with centroid-based region classification so early groups can be forced by geography (for example, Americas first, then Asia, then Africa/Europe) while preserving random grouping option.
- [Done] 2026-02-28: Added longitude-chunk countries grouping strategy in `assets/pages/overview/globe-v3.7e.html` (`countriesGroupStrategy: longitude`, `countriesLongitudeBandsCsv`, `countriesLongitudeOrderCsv`, `countriesGroupPhaseDelayMs`) so reveal order can follow visibility-oriented longitude sections (for example, loading Oceania with West Asia in the same phase).
- [Done] 2026-02-28: Tuned default longitude-group reveal settings in `assets/pages/overview/globe-v3.7e.html` for clearer phased behavior (`countriesGroupPhaseDelayMs: 1200`, `countriesGroupCount: 12`, `countriesGroupStaggerMs: 260`, `countriesFadeDurationMs: 450`).
...
- [Done] 2026-03-01: Built Portal V2-only backend/dashboard enhancement without modifying legacy `form-backend/apps_script_webapp.js`: added route-tab new-data signaling (`setTabColor`), per-route unread/reviewed state in Script Properties, dashboard tab generation (`portal_v2_dashboard` default), and review reset helpers (`portalV2MarkRouteReviewed`, `portalV2MarkAllReviewed`) in `form-backend/portal_v2_apps_script_webapp.js` + new dashboard config keys in `form-backend/portal_v2_config.js`; documented deployment/usage in `PORTAL-V2-README.md`.
- [Done] 2026-03-01: Upgraded `form-backend/portal_v2_apps_script_webapp.js` into a unified cutover-ready endpoint that supports both legacy (`stakeholder`, `investor`, `employment`) and Portal V2 (`investment`, `press`, `employment`, `internship`) submission logic with route-aware sheet mapping and schema-specific row construction, while leaving legacy `form-backend/apps_script_webapp.js` unchanged for testing/rollback; added legacy routing property keys in `form-backend/portal_v2_config.js` and updated migration notes in `PORTAL-V2-README.md`.
- [Done] 2026-03-01: Added V2 per-route auto-reply controls and route-level Drive folder overrides (including legacy per-route folder keys) in `form-backend/portal_v2_config.js` + `form-backend/portal_v2_apps_script_webapp.js`; auto-reply can now be globally enabled and toggled independently per route, while attachment metadata columns remain part of both legacy and portal sheet schemas.
- [Done] 2026-03-01: Cut over site form endpoints to new `TSI Intake v2` web app deployment (`AKfycbwDGGSMwLCjaU3krrwZl4rrNL2EvLIIwtxz3XK93JXV0eDbL3VQF-MWFVMwPh2oRrgGGg/exec`) by updating `js/main.js` (`FORM_ENDPOINT`) and `js/portal-intake-v2.js` (`PORTAL_V2_ENDPOINT`) to the same unified V2 endpoint.
